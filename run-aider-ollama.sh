...
#   ./run-aider-ollama.sh --gen-metadata     # (re)write the aider model metadata files
#
# Run --gen-metadata once, and again whenever the catalogue changes. It asks
# Ollama.com for each model's real context length so aider stops guessing; the
# two files it writes sit next to this script and are passed to aider on launch.
#
...
# Aider has no built-in facts about Ollama.com models, so it guesses context
# windows and edit format. These two files tell it the truth; regenerate them
# from the API with --gen-metadata whenever the catalogue changes.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
META_JSON="$SCRIPT_DIR/ollama-model-metadata.json"
META_YML="$SCRIPT_DIR/ollama-model-settings.yml"
...
if [ "$LIST_ONLY" -eq 0 ] && [ -z "${GEN_META:-}" ]; then
...
# ---------------------------------------------------------------- metadata
# /api/show reports each model's real context length and parameter count. Aider
# knows neither (these models are not in litellm's database), so without this it
# warns "Unknown context window size and costs" and falls back to defaults.
if [ -n "${GEN_META:-}" ]; then
  command -v jq >/dev/null || die "--gen-metadata needs jq"
  show_dir="$(mktemp -d)"
  trap 'rm -rf "$show_dir"' EXIT

  info "asking $TAGS_URL's /api/show about ${#NAMES[@]} models ..."
  i=0
  for name in "${NAMES[@]}"; do
    curl -s --max-time 30 -X POST "$OLLAMA_CLOUD_HOST/api/show" \
      -H "Authorization: Bearer $OLLAMA_API_KEY" -H 'Content-Type: application/json' \
      -d "{\"model\":\"$name\"}" > "$show_dir/$i.json" &
    i=$((i+1))
    [ $((i % 8)) -eq 0 ] && wait
  done
  wait
  ...
  : > "$show_dir/entries"
  {
    echo "# Aider model settings for Ollama.com — generated by run-aider-ollama.sh --gen-metadata"
    echo "# edit_format: diff for large models, whole for small ones (they follow diffs badly)."
  } > "$META_YML"

  i=0; skipped=0
  for name in "${NAMES[@]}"; do
    f="$show_dir/$i.json"; i=$((i+1))
    ctx="$(jq -r '[(.model_info // {}) | to_entries[] | select(.key|endswith(".context_length")) | .value][0] // empty' "$f" 2>/dev/null || true)"
    params="$(jq -r '(.model_info["general.parameter_count"] // .details.parameter_size // 0) | tonumber? // 0' "$f" 2>/dev/null || echo 0)"
    if [ -z "$ctx" ] || [ "$ctx" = "null" ]; then
      info "  no context length reported for $name — leaving it to aider's defaults"
      skipped=$((skipped+1))
      continue
    fi

    # Costs are 0 because Ollama.com bills by plan, not per token — aider would
    # otherwise print invented dollar amounts.
    jq -n --arg k "openai/$name" --argjson ctx "$ctx" '{
      ($k): {
    ...
    if [ "$params" -ge 100000000000 ] 2>/dev/null; then ef="diff"; else ef="whole"; fi
    printf -- '- name: openai/%s\n  edit_format: %s\n  use_repo_map: true\n' "$name" "$ef" >> "$META_YML"
  done

  jq -s 'add' "$show_dir/entries" > "$META_JSON"
  info "wrote $META_JSON"
  info "wrote $META_YML"
  [ "$skipped" -gt 0 ] && info "($skipped model(s) reported no context length)"
  exit 0
fi
...
META_ARGS=()
if [ -r "$META_JSON" ]; then
  META_ARGS+=(--model-metadata-file "$META_JSON")
else
  info "no $META_JSON — run '$0 --gen-metadata' to stop aider guessing context sizes"
fi
[ -r "$META_YML" ] && META_ARGS+=(--model-settings-file "$META_YML")
...
exec aider --model "$AIDER_MODEL" "${META_ARGS[@]}" "${AIDER_ARGS[@]}"
