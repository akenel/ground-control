# AGENTS.md — Ground Control

*If you are an AI agent reading this — Claude Code, Codex, Cursor, aider, Gemini CLI, opencode, goose, whichever — this file tells you what this repo is and how to help the person who just opened it. It is the whole spine; `CLAUDE.md` is a two-line pointer at it.*

*If you are a human reading this: this file is also a live example of the very thing the kit teaches — a filled-in context spine, saved under the [open format's](https://agents.md/) name. Steal the shape.*
...
├── AGENTS.md         # this file — the spine; onboards any agent opening the repo
├── CLAUDE.md         # a pointer to AGENTS.md, so Claude Code loads it too
├── LICENSE           # CC BY 4.0
...
- **2026-08-15 — The kit was named for one vendor's tool while promising "no vendor."** The
  template was `CLAUDE.template.md` and the spine was `CLAUDE.md`, in a repo whose whole claim is
  that you own your context rather than rent it. `AGENTS.md` is an open format read by 20+ agents
  across 60k+ projects, so the canonical files now carry that name and `CLAUDE.md` is a pointer.
  **Proven before it was claimed:** the same spine was run as `CLAUDE.md` in Claude Code, as an
  Open-WebUI system prompt, and as `CONVENTIONS.md` driving aider with Qwen on Ollama — where it
  turned silently-written pointless code into a reasoned refusal, with only the spine changed.
